    ...
    for epoch in range(restart_epoch,hp.epochs):
        for i, batchs in enumerate(training_loader):
            ...
            for j, batch in enumerate(batchs):
                ...
                # Backward
                e_t=time.perf_counter()
                time_dict['logs']+=(e_t-s_t)
                
                s_t=time.perf_counter()
                total_loss.backward()
                e_t=time.perf_counter()
                time_dict['back']+=(e_t-s_t)
                ...
                e_t=time.perf_counter()
                time_dict['time']+=(e_t-s_t)

                if iteration%500==0:
                    time_dict['total']+=time.perf_counter()-sta
                    print(time_dict)
                    exit()
                iteration+=1
